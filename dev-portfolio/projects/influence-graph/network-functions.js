...
class Graph {
    constructor({ canvas, nodes = [], edges = [], weightFactor = 1, primaryDark = "#90a4ae", primaryMid = "#b0bec5", primaryLight = "#fce4ec", secondaryDark = "#ffab91", secondaryMid = "#f06292", tertiaryMid = "#9575cd", }) {
        ...
        edges
            .filter((e) => this.nodeIds.includes(e.source) &&
    ...
    drawLine({ canvas, source, target, color, weight = 1, }) {
    ...
    drawCircle({ canvas, x, y, id, weight, fillColor, strokeColor, label = "", displayLabel = false, highlighted = false, strokeWidth = 2, }) {
        const ns = "http://www.w3.org/2000/svg";
    ...
    draw() {
        this.edges.forEach((e) => this.drawLine({
            canvas: this.canvas,
            source: e.source,
            target: e.target,
        ...
        this.nodes.forEach((n) => this.drawCircle({
            canvas: this.canvas,
            x: n.x,
            y: n.y,
            id: n.id,
