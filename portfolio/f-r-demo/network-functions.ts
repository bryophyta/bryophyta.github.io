...
class Graph {
    ...
    addNode({
        ...
        strokeColor = null,
        fillColor = null
    }: {id: string, x?: number | null, y?: number | null, strokeColor?: string | null, fillColor?: string | null}){
    ...
    fa(z: number, k: number, c: number): number {return (Math.pow(z, 2) / k) * c;}
    fr(z: number, k:number, c: number): number {return (Math.pow(k, 2) / z) * c;}
    dist(vector: {x: number, y: number}){return Math.sqrt(Math.pow(vector.x, 2) + Math.pow(vector.y, 2))}
    ...
    async fruchtermanReingoldAnimate(attractionConstant: number, repulsionConstant: number){
        this.playing = true;
        const net = {nodes: this.nodes, edges: this.edges};
        const width = this.width;
        const height = this.height;
    ...
    containsNode(edge: {source: GraphNode, target: GraphNode}, node: GraphNode){
    ...
    shortestPath(startId: string, endId: string){
        ...
        if (!startNode || !endNode) throw "Path can only be calculated between two nodes in the network."
    ...
    breadthFirstSearch(edges: {source: GraphNode, target: GraphNode}[], queue: {node: GraphNode, depth: number}[], endNode: GraphNode, covered: GraphNode[]): number {
        if (queue.length > 0){
...
function drawLine({
            ...
            {canvas: HTMLElement, source: GraphNode, target: GraphNode, color: string, weight: number}
            )
            {
